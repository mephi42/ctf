...
"""
        ...
        def check_smallbin(exp_smallbin_fd, exp_smallbin_bk):
            ...
            smallbin_bk = int(
                tube.gdb.parse_and_eval("*(int*)(__vmctx->memory+0x13b8+16)")
            )
        ...
        if args.LOCAL and not args.NOPTRACE:
            ...
            check_smallbin(0x11EB8, 0x13B8 + 8)
            tube.gdb.continue_nowait()
...
"""
        ...
        if args.LOCAL:
            tube.gdb.interrupt_and_wait()
            check_smallbin(PATH_ADDR - PATH_OFFSET - 8, 0x13B8 + 8)
            tube.gdb.continue_nowait()
